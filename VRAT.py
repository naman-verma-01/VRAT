...
with mp_hands.Hands(min_detection_confidence=0.6, min_tracking_confidence=0.2) as hands:
    while True:

        img = requests.get(url)
        img_arr = np.array(bytearray(img.content), dtype=np.uint8)
        img = cv2.imdecode(img_arr, -1)
        image = imutils.resize(img, width=640, height=488)
        ...
        try:
            if lh[8][2] < lh[6][2] and lh[12][2] > lh[10][2] and lh[16][2] > lh[14][2] and lh[20][2] > lh[18][2]:
                value = "one"
            elif lh[8][2] < lh[6][2] and lh[12][2] < lh[10][2] and lh[16][2] > lh[14][2] and lh[20][2] > lh[18][2]:
                value = "two"
            else:
                value = " "
        except:
